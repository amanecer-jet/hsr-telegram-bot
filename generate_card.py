...
def generate_profile_card(profile: Dict, uid: str, char_idx: int = 0) -> Image.Image:
    ...
    # --- Портрет  --
    port_url = ch.get("portrait") or ch.get("preview") or ch.get("icon")
    portrait = _download_image(port_url, (LEFT_W-20, LEFT_W-20))
    canvas.paste(portrait, (10, 10), portrait)
    ...
    if isinstance(attrs, list):
        attr_map = {item.get("field") or item.get("type") or item.get("name"): item.get("display") or item.get("value") for item in attrs}
    else:
        attr_map = {k: (v.get("display") if isinstance(v, dict) else v) for k, v in attrs.items()}
    ...
    for idx, (field, label) in enumerate(stat_order):
        val = attr_map.get(field) or attr_map.get(label) or "-"
