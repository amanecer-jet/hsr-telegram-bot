...
class HSRDataParser:
    ...
    def fetch_character_build_selenium(self, slug: str) -> dict:
        """Парсит билд персонажа с prydwen.gg/star-rail/characters/{slug}/ через Selenium, если нет данных в JSON"""
        ...
        try:
            driver.get(url)
            time.sleep(3)  # Ждём рендеринга JS
            soup = BeautifulSoup(driver.page_source, 'lxml')
            build = {}
            # Пример поиска секций по заголовкам (можно доработать под реальную структуру)
            # Relics
            relics_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Relics' in tag.text)
            if relics_header:
                relics_list = relics_header.find_next('ul')
                if relics_list:
                    build['relics'] = [li.get_text(strip=True) for li in relics_list.find_all('li')]
            # Light Cones
            cones_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Light Cones' in tag.text)
            if cones_header:
                cones_list = cones_header.find_next('ul')
                if cones_list:
                    build['light_cones'] = [li.get_text(strip=True) for li in cones_list.find_all('li')]
            # Main Stats
            main_stats_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Main Stats' in tag.text)
            if main_stats_header:
                stats_list = main_stats_header.find_next('ul')
                if stats_list:
                    build['main_stats'] = [li.get_text(strip=True) for li in stats_list.find_all('li')]
            # Sub Stats
            sub_stats_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Sub Stats' in tag.text)
            if sub_stats_header:
                stats_list = sub_stats_header.find_next('ul')
                if stats_list:
                    build['sub_stats'] = [li.get_text(strip=True) for li in stats_list.find_all('li')]
            # Priority
            priority_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Priority' in tag.text)
            if priority_header:
                priority_list = priority_header.find_next('ul')
                if priority_list:
                    build['priority'] = [li.get_text(strip=True) for li in priority_list.find_all('li')]
            # Guide (ищем большой текстовый блок)
            guide_header = soup.find(lambda tag: tag.name in ['h2', 'h3'] and 'Guide' in tag.text)
            if guide_header:
                guide_block = guide_header.find_next('div')
                if guide_block:
                    build['guide'] = guide_block.get_text(strip=True)[:1500]
            return build
        except Exception as e:
            logger.error(f"Ошибка парсинга билдов через Selenium для {slug}: {e}")
            return {"error": str(e)}
        finally:
            driver.quit()

    def fetch_character_build(self, slug: str) -> dict:
        """Получает билд персонажа с prydwen.gg через JSON, поддерживает оба формата: currentUnit и contentfulHsrCharacter. Если данных нет, использует Selenium."""
        ...
        try:
            ...
            if 'guide' in char_data and isinstance(char_data['guide'], str) and char_data['guide']:
                result['guide'] = char_data['guide']
            if 'review' in char_data and isinstance(char_data['review'], str) and char_data['review']:
                result['review'] = char_data['review']
            # 4. Если ничего не найдено — fallback на Selenium
            if not result:
                logger.info(f"[Prydwen] Fallback на Selenium для {slug}")
                result = self.fetch_character_build_selenium(slug)
            if result:
                return result
            return {"error": "No build or recommendations found in JSON or HTML."}
        except Exception as e:
