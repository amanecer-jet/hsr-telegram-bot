...
async def fetch_build_from_game8(character_name: str) -> dict:
    ...
    if cones_header:
        ...
        for cone in cones:
            if "3★" in cone or "3*" in cone or "3-star" in cone.lower():
                ...
                break
        # Если не нашли 3★ конус, используем стандартное значение
    ...
    if stats_header:
        ...
        if section:
            if section.name == "table":
                # Ищем таблицу с основными характеристиками
                for row in section.find_all("tr"):
                    cells = row.find_all("td")
                    if len(cells) >= 2:
                        slot = cells[0].get_text(strip=True).lower()
                        stat = cells[1].get_text(strip=True)
                        if "body" in slot:
                            build["stats"]["main"]["body"] = stat
                        elif "feet" in slot:
                            build["stats"]["main"]["feet"] = stat
                        elif "sphere" in slot:
                            build["stats"]["main"]["sphere"] = stat
                        elif "rope" in slot:
                            build["stats"]["main"]["rope"] = stat
    ...
    if planar_header:
        ...
        build["ornaments"]["best"] = "Warrior Goddess of Sun and Thunder"
