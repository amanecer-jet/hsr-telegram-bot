...
async def cmd_admin_post(message: types.Message, state: FSMContext):
    if not ADMIN_CHAT_ID or str(message.from_user.id) != str(ADMIN_CHAT_ID):
        ...
        return
    await message.reply("Отправьте сообщение (текст, фото, видео, стикер и т.д.), которое нужно разослать всем подписчикам.")
...
async def admin_broadcast(message: types.Message, state: FSMContext):
    ...
    for chat_id in subs:
        try:
            # Копируем исходное сообщение во все чаты — так сохраняются все медиа и форматирование
