...
async def send_uid_card(uid: str, chat_id: int):
    # 1) Пробуем полный профиль через Mihomo
    profile_raw = fetch_profile(int(uid))
    img = None
    if profile_raw and profile_raw.get("characters"):
        try:
            img = generate_profile_card(profile_raw, uid)
        except Exception:
            img = None

    if img is None:
        # fallback к упрощённой карточке
    ...
    await bot.send_photo(chat_id=chat_id, photo=photo_input)
