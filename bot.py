...
# Новый форматтер для структурированной выдачи билдов

def format_build_v2(character_name, build):
    lines = []
    lines.append(f"*Билд для {md.quote(character_name)}*")
    if build.get("relics_recommended"):
        lines.append("\n*Рекомендуемые реликвии:*\n" + mdv2_list(build["relics_recommended"]))
    if build.get("relics_alternative"):
        lines.append("\n*Альтернативные реликвии:*\n" + mdv2_list(build["relics_alternative"]))
    cones = build.get("cones", {})
    if cones.get("5") or cones.get("4") or cones.get("other"):
        lines.append("\n*Световые конусы:*")
        if cones.get("5"):
            lines.append("⭐️ 5★:" + ("\n" + mdv2_list(cones["5"]) if cones["5"] else ""))
        if cones.get("4"):
            lines.append("⭐️ 4★:" + ("\n" + mdv2_list(cones["4"]) if cones["4"] else ""))
        if cones.get("other"):
            lines.append("Другие:" + ("\n" + mdv2_list(cones["other"]) if cones["other"] else ""))
    if build.get("main_stats"):
        lines.append("\n*Основные характеристики:*\n" + mdv2_list(build["main_stats"]))
    if build.get("sub_stats"):
        lines.append("\n*Второстепенные характеристики:*\n" + mdv2_list(build["sub_stats"]))
