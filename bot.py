...
class HSRDataParser:
    ...
    def fetch_character_build(self, slug: str) -> dict:
        """Парсит билд персонажа с prydwen.gg/star-rail/characters/{slug}/"""
        url = f"{self.base_url}/star-rail/characters/{slug}/"
        try:
            ...
            # Relics
            relics_block = soup.find('div', string=lambda s: s and 'Relics' in s)
            if relics_block:
                relics_list = relics_block.find_next('ul')
                if relics_list:
                    build['relics'] = [li.get_text(strip=True) for li in relics_list.find_all('li')]

            # Light Cones
            cones_block = soup.find('div', string=lambda s: s and 'Light Cones' in s)
            if cones_block:
                cones_list = cones_block.find_next('ul')
                if cones_list:
                    build['light_cones'] = [li.get_text(strip=True) for li in cones_list.find_all('li')]

            # Main Stats
            main_stats_block = soup.find('div', string=lambda s: s and 'Main Stats' in s)
            if main_stats_block:
                stats_list = main_stats_block.find_next('ul')
                if stats_list:
                    build['main_stats'] = [li.get_text(strip=True) for li in stats_list.find_all('li')]

            # Sub Stats
            sub_stats_block = soup.find('div', string=lambda s: s and 'Sub Stats' in s)
            if sub_stats_block:
                stats_list = sub_stats_block.find_next('ul')
                if stats_list:
                    build['sub_stats'] = [li.get_text(strip=True) for li in stats_list.find_all('li')]

            # Priority
            priority_block = soup.find('div', string=lambda s: s and 'Priority' in s)
            if priority_block:
                priority_list = priority_block.find_next('ul')
                if priority_list:
