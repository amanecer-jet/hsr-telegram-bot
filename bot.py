...
bot = Bot(token=API_TOKEN, default=DefaultBotProperties(parse_mode="MarkdownV2", server_side_access_token=API_TOKEN))
                ...
                for row in section.find_all("tr"):
                    ...
                    if len(cells) >= 2:
                        ...
                        for slot, keywords in ParsingConfig.STATS_MAIN_SLOTS.items():
                            if any(keyword in slot_text for keyword in keywords):
                                build["stats"]["main"][slot] = stat
                                break
                            parts = text.split(" - ")
                            if len(parts) >= 2:
                                rating = parts[0].strip()
                                stat = parts[1].strip()
                                # Если рейтинг содержит звезды, считаем их
                                if "★" in rating:
                                    rating = rating.count("★")
                                elif "star" in rating.lower():
                                    # Если рейтинг указан как "3-star", "4-star" и т.д.
                                    rating = int(rating.split("-star")[0])
