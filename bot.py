...
async def fetch_build_from_game8(character_name: str) -> dict:
    ...
    build = {"relics": [], "cones": [], "stats": []}
    # --- Relics ---
    relic_header = soup.find(lambda tag: tag.name in ["h2", "h3"] and ("Relic" in tag.text or "Ornament" in tag.text))
    if relic_header:
        relic_section = relic_header.find_next(["ul", "table"])
        if relic_section:
            if relic_section.name == "ul":
                build["relics"] = [li.get_text(strip=True) for li in relic_section.find_all("li")]
            elif relic_section.name == "table":
                for row in relic_section.find_all("tr"):
                    cells = row.find_all("td")
                    if cells:
                        build["relics"].append(cells[0].get_text(strip=True))
        else:
            logging.warning(f"Не найден список/таблица реликвий для {character_name}")
    else:
        logging.warning(f"Не найден заголовок реликвий для {character_name}")
    # --- Light Cones ---
    cone_header = soup.find(lambda tag: tag.name in ["h2", "h3"] and "Light Cone" in tag.text)
    if cone_header:
        cone_section = cone_header.find_next(["ul", "table"])
        if cone_section:
            if cone_section.name == "ul":
                build["cones"] = [li.get_text(strip=True) for li in cone_section.find_all("li")]
            elif cone_section.name == "table":
                for row in cone_section.find_all("tr"):
                    cells = row.find_all("td")
                    if cells:
                        build["cones"].append(cells[0].get_text(strip=True))
        else:
            logging.warning(f"Не найден список/таблица конусов для {character_name}")
    else:
        logging.warning(f"Не найден заголовок конусов для {character_name}")
    # --- Main Stats ---
    stats_header = soup.find(lambda tag: tag.name in ["h2", "h3"] and ("Main Stat" in tag.text or "Main Stats" in tag.text))
    if stats_header:
        stats_section = stats_header.find_next(["ul", "table"])
        if stats_section:
            if stats_section.name == "ul":
                build["stats"] = [li.get_text(strip=True) for li in stats_section.find_all("li")]
            elif stats_section.name == "table":
                for row in stats_section.find_all("tr"):
                    cells = row.find_all("td")
                    if cells:
                        build["stats"].append(cells[0].get_text(strip=True))
        else:
            logging.warning(f"Не найден список/таблица характеристик для {character_name}")
    else:
        logging.warning(f"Не найден заголовок характеристик для {character_name}")
