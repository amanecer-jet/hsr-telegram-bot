...
CHARACTER_BASE_URL = "https://prydwen.gg/hsr/characters/"
...
async def fetch_characters_and_elements() -> dict:
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Ждем загрузки контента (3 секунды должно хватить)
            ...
            # Используем правильный способ инициализации BeautifulSoup
            soup = BeautifulSoup(html, features="html.parser")
            
            # Логируем структуру HTML для отладки
            logging.info("Структура HTML:")
            ...
            if not characters_container:
                logging.info("Используем поиск по структуре...")
                # Ищем все div элементы с классами
                for div in soup.find_all("div", class_=True):
                    classes = div.get("class")
                    if isinstance(classes, list):
                        for cls in classes:
                            if "character" in cls.lower():
                ...
                possible_classes = ["characters-list", "characters-container", "characters-grid"]
                
                for cls in possible_classes:
                    characters_container = soup.find("div", class_=cls)
                    if characters_container:
                        logging.info(f"Найден контейнер с классом {cls}")
                        break
                
                if not characters_container:
                    # Если не нашли по классу, ищем по структуре
                    logging.info("Используем поиск по структуре...")
                    characters_container = soup.find("div", recursive=True)
                    if characters_container:
                        logging.info("Найден контейнер по структуре")
                    else:
            ...
            char_cards = characters_container.find_all("div", recursive=True)
            logging.info(f"Найдено {len(char_cards)} потенциальных карточек персонажей")
            ...
            for char_card in char_cards:
                ...
                # Проверяем, что это не ссылка на главную или политику
                if char_name.lower() in ["home", "privacy policy", "back to top"]:
                    continue
                
                # Проверяем, что это действительно персонаж
                if not any(word in char_name.lower() for word in ["character", "build", "guide"]):
                    continue
