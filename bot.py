...
def format_build(build):
    name = escape_md(build.get("name", ""))
    build_type = escape_md(build.get("build_type", ""))
    weapon = escape_md(build.get("weapon", ""))
    artifacts = build.get("artifacts", {})
    art_str = "\n".join([f"*{escape_md(k)}*: {escape_md(v)}" for k, v in artifacts.items()])
    return (
        f"*{name}*\n"
        f"_{build_type}_\n"
        f"*Оружие*: {weapon}\n"
