...
def get_relic_name(game_data, relic_id):
    for relic in game_data["relics"]:
        if relic["id"] == relic_id:
            return relic["name"]
    return relic_id

def get_planar_name(game_data, relic_id):
    # Планарные украшения — это реликвии типа "Planar"
    for relic in game_data["relics"]:
        if relic["id"] == relic_id and relic.get("type") == "Planar":
            return relic["name"]
    return relic_id
...
def get_main_stat_name(game_data, stat_id):
    for stat in game_data["relic_main_affixs"]:
        if stat["id"] == stat_id:
            return stat["name"]
    return stat_id

def get_sub_stat_name(game_data, stat_id):
    for stat in game_data["relic_sub_affixs"]:
        if stat["id"] == stat_id:
            return stat["name"]
    return stat_id
...
def format_build(character, game_data):
    ...
    if relics:
        msg += f"<b>Рекомендуемые реликвии:</b> {get_relic_name(game_data, relics[0])}\n"
        if len(relics) > 1:
            msg += f"<b>Альтернатива:</b> {get_relic_name(game_data, relics[1])}\n"
