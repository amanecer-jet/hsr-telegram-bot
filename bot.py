...
def game_keyboard(subscribed: bool = False):
    kb = [
        ...
        [InlineKeyboardButton(text="🛠 Zenless Zone Zero (WIP)", callback_data="game:Zenless Zone Zero")],
...
async def cb_choose_game(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"<b>{game}</b> – выберите действие:",
        reply_markup=feature_keyboard(game)
    )
...
async def cb_choose_feature(callback: types.CallbackQuery, state: FSMContext):
    ...
    parts = callback.data.split(":", 2)  # feature:<game>:<feat>
    if len(parts) < 3:
        await callback.answer()
        return
    _prefix, game, feat = parts
    if feat == "builds":
        if game == "Zenless Zone Zero":
            await callback.message.edit_text("Функция в разработке. Пожалуйста, загляните позже!", reply_markup=feature_keyboard(game))
            return
