...
def load_cache() -> dict:
    ...
    Возвращает словарь с данными кэша или пустой словарь в случае ошибки.
    """
    default_cache = {
        "elements": {},
        "characters": {},
    ...
    try:
        if not os.path.exists(CACHE_FILE):
            logging.info(f"Файл кэша {CACHE_FILE} не найден, будет создан новый")
        ...
        if not isinstance(cache_data, dict):
            raise ValueError("Некорректный формат кэша: ожидался словарь")
            
        # Объединяем с дефолтными значениями на случай отсутствия ключей
        result = {**default_cache, **cache_data}
        
        # Проверяем наличие обязательных ключей
        for key in ["elements", "characters", "links"]:
            if key not in result or not isinstance(result[key], dict):
                result[key] = {}
                
        logging.debug("Кэш успешно загружен")
        return result
        
    except json.JSONDecodeError as e:
        logging.error(f"Ошибка декодирования JSON в файле {CACHE_FILE}: {str(e)}")
    except Exception as e:
        logging.error(f"Ошибка при загрузке кэша из {CACHE_FILE}: {str(e)}")
...
def save_cache(data: dict) -> bool:
    ...
    if not isinstance(data, dict):
        logging.error("Некорректные данные для сохранения: ожидался словарь")
        return False
        
    try:
        # Создаем директорию, если её нет
        cache_dir = os.path.dirname(CACHE_FILE)
        if cache_dir:  # Проверяем, что путь не пустой
        ...
        if os.path.getsize(temp_file) == 0:
            raise ValueError("Не удалось сохранить кэш: файл пуст")
        
        # Заменяем старый файл новым (атомарная операция в Windows)
        if os.path.exists(CACHE_FILE):
            os.replace(temp_file, CACHE_FILE)
        else:
            os.rename(temp_file, CACHE_FILE)
            
        logging.debug("Кэш успешно сохранен")
        return True
        
    except (IOError, OSError) as e:
        logging.error(f"Ошибка ввода-вывода при сохранении кэша: {str(e)}")
    except json.JSONEncodeError as e:
        logging.error(f"Ошибка кодирования JSON при сохранении кэша: {str(e)}")
    except Exception as e:
        logging.error(f"Неожиданная ошибка при сохранении кэша: {str(e)}")
...
async def update_full_cache():
    ...
    try:
        logging.info("Начало обновления кэша...")
        # Создаем директорию data, если её нет
        os.makedirs('data', exist_ok=True)
        
        logging.info("Получение данных о персонажах...")
        characters_data = await fetch_characters_and_elements()
        
        if not characters_data or not characters_data.get("characters"):
            logging.error("Не удалось получить данные о персонажах")
            return False
        ...
        if os.path.exists('data/cache.json') and os.path.getsize('data/cache.json') > 0:
            logging.info("Кэш успешно обновлен и сохранен")
            return True
        else:
            logging.error("Не удалось сохранить кэш: файл пуст или не создан")
            return False
            
    except Exception as e:
        logging.error(f"Критическая ошибка при обновлении кэша: {str(e)}", exc_info=True)
        return False
...
async def fetch_characters_and_elements() -> dict:
    ...
    try:
        logging.info("Запрос списка персонажей...")
        async with async_playwright() as p:
            # Запускаем браузер с дополнительными настройками
            ...
            # Создаем новую страницу с мобильными настройками
            context = await browser.new_context(
                viewport={"width": 414, "height": 896},  # iPhone 8 размер
            ...
            # Переходим на страницу с простыми настройками
            logging.info(f"Открываем URL: {CHARACTER_BASE_URL}")
            try:
                ...
                response = await page.goto(CHARACTER_BASE_URL, wait_until="networkidle")
                ...
                logging.info(f"Получен HTML: {html[:500]}")  # Логируем первые 500 символов
                ...
                errors = await page.evaluate("""
                    () => {
                        ...
                        window.onerror = function(msg, url, line, col, error) {
                            errors.push({msg, url, line, col, error: error && error.message});
                        };
                        return errors;
                    }
                """)
                if errors:
                    logging.error(f"JavaScript ошибки: {errors}")
                if "Error" in html or "Not Found" in html:
                    logging.error("Ошибка загрузки страницы")
                    return {"elements": {}, "characters": {}, "links": {}}
                
                # Пытаемся найти контейнер с персонажами
                container = None
                selectors = [
                    "div.character-card",
                    "div.character-grid",
                ...
                for selector in selectors:
                    try:
                        ...
                        if container:
                            logging.info(f"Найден контейнер по селектору: {selector}")
                            break
                    except:
                        continue
                ...
                # Логируем все классы на странице
                logging.info("Классы на странице:")
                elements = await page.query_selector_all("*")
                for element in elements:
                    try:
                        classes = await element.get_attribute("class")
                        if classes:
                            logging.info(f"Найден элемент с классами: {classes}")
                    except:
                        continue
                
                await browser.close()
            except Exception as e:
                logging.error(f"Ошибка при ожидании контейнера: {str(e)}")
                # Попытаемся найти контейнер по другим классам
                html = await page.content()
                await browser.close()
                soup = BeautifulSoup(html, features="html.parser")
                
                # Логируем все div элементы на странице
                logging.info("Поиск контейнера по всем div элементам:")
                for div in soup.find_all("div", class_=True):
                    classes = div.get("class", [])
                    logging.info(f"Найден div с классами: {classes}")
            ...
            logging.info("Получен HTML, начинаем парсинг...")
            soup = BeautifulSoup(html, features="html.parser")
            
            # Логируем структуру страницы
            logging.info("Структура страницы:")
            for tag in soup.find_all(True, limit=50):  # Увеличиваем лимит до 50 элементов
                logging.info(f"Тег: {tag.name}, классы: {tag.get('class', [])}, атрибуты: {dict(tag.attrs)}")
            
            # Логируем все div элементы на странице
            logging.info("Все div элементы:")
            divs = soup.find_all("div")
            logging.info(f"Найдено всего div элементов: {len(divs)}")
            
            # Ищем контейнер с персонажами
            characters_container = None
            
            # Пытаемся найти контейнер по структуре страницы
            # Ищем все div элементы с классами
            all_divs = soup.find_all("div", class_=True)
            for div in all_divs:
                # Проверяем, содержит ли div карточки персонажей
                cards = div.find_all("div", recursive=False)  # Ищем прямые дочерние div
                if len(cards) > 10:  # Если больше 10 карточек, это вероятно наш контейнер
                    logging.info(f"Найден потенциальный контейнер с {len(cards)} карточками")
                    characters_container = div
                    break
            
            if not characters_container:
                # Пытаемся найти по другим признакам
                # Ищем div с текстом "Characters"
                title_div = soup.find("div", text=lambda t: t and "Characters" in t)
                if title_div:
                    # Ищем следующий div после заголовка
                    next_div = title_div.find_next_sibling("div")
                    if next_div:
                        logging.info("Найден контейнер по заголовку Characters")
                        characters_container = next_div
            
            if not characters_container:
                # Пытаемся найти по другим признакам
                # Ищем div с изображениями персонажей
                for img in soup.find_all("img"):
                    if "character" in str(img.get("alt", "")):
                        # Ищем родительский контейнер изображения
                        container = img.find_parent("div")
                        if container:
                            logging.info("Найден контейнер по изображению персонажа")
                            characters_container = container
                            break
            ...
            char_cards = characters_container.find_all("div", class_="character-card")
            logging.info(f"Найдено {len(char_cards)} карточек персонажей")
            
            for char_card in char_cards:
                # Ищем ссылку на персонажа
                char_a = char_card.find("a", href=True)
                if not char_a:
                    continue
                ...
                char_link = CHARACTER_BASE_URL + char_a["href"].split('/')[-1]
                
                # Ищем элемент персонажа
                element_img = char_card.find("img", alt=True)
                if not element_img:
                ...
                logging.debug(f"Добавлен персонаж: {char_name} ({element})")
            ...
            if not characters:
                logging.error("Не найдено ни одного персонажа!")
                return {"elements": {}, "characters": {}, "links": {}}
            
            return {"elements": elements, "characters": characters, "links": links}
            
    except Exception as e:
        logging.error(f"Ошибка при загрузке списка персонажей: {str(e)}", exc_info=True)
        return {"elements": {}, "characters": {}, "links": {}}
...
def is_cache_expired(cache_data: dict, ttl_days: int = 10) -> bool:
    ...
    if not last_updated or not isinstance(last_updated, str):
        logging.warning("last_updated не является строкой в кэше")
        return True
    
    try:
        last_updated_dt = datetime.fromisoformat(last_updated)
        return (datetime.now() - last_updated_dt) > timedelta(days=ttl_days)
    except (ValueError, TypeError) as e:
        logging.error(f"Ошибка при преобразовании даты в кэше: {e}")
        return True
    ...
    while True:
        try:
            ...
            if is_cache_expired(cache) or not cache.get("elements"):
                logging.info("Обновление кэша...")
                char_data = await fetch_characters_and_elements()
                if char_data and "elements" in char_data:
                    cache["elements"] = char_data.get("elements", {})
                    cache["characters"] = char_data.get("characters", {})
                    cache["links"] = char_data.get("links", {})
                    cache["last_updated"] = datetime.now().isoformat()
                    save_cache(cache)
                ...
                logging.info("Кэш актуален, пропускаем обновление")
        except Exception as e:
            logging.error(f"Ошибка в cache_auto_updater: {str(e)}", exc_info=True)
...
dp.message.register(cmd_build, Command(commands=['build']))
dp.message.register(cmd_update_cache, Command(commands=['update_cache']))
...
dp.message.register(choose_game, BuildStates.choose_game)
dp.message.register(choose_element, BuildStates.choose_element)
dp.message.register(choose_character, BuildStates.choose_character)
...
async def main():
    ...
    try:
        ...
        cache = load_cache()
        if not cache.get("elements") or is_cache_expired(cache):
            logging.warning("Кэш пуст или устарел, выполняется принудительное обновление...")
            await cache_auto_updater()
