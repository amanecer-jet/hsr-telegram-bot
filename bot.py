...
class HSRDataParser:
    ...
    def fetch_character_build(self, slug: str) -> dict:
        """Получает билд персонажа с prydwen.gg через JSON"""
        url = f"{self.base_url}/page-data/star-rail/characters/{slug}/page-data.json"
        try:
            resp = requests.get(url, timeout=10)
            resp.raise_for_status()
            data = resp.json()
            char_data = data.get('result', {}).get('data', {}).get('contentfulHsrCharacter', {})
            builds = char_data.get('builds', [])
            if not builds:
                return {"error": "No builds found in JSON."}
            # Берём первый билд (обычно основной)
            build = builds[0]
            result = {}
            if 'relics' in build:
