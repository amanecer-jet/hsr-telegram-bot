...
async def start_webhook():
    ...
    # --- HoYoLAB watcher: уведомления о новостях/событиях ---
    try:
        from hoyolab_watcher import start_hoyolab_watcher
        # Куда отправлять уведомления: переменная окружения HOYOLAB_NOTIFY_CHAT либо ADMIN_CHAT_ID из .env
        target_chat = int(os.getenv("HOYOLAB_NOTIFY_CHAT", os.getenv("ADMIN_CHAT_ID", "0")))
        if target_chat:
            asyncio.create_task(start_hoyolab_watcher(bot))
    except Exception as e:
        logging.warning(f"[bot] Не удалось запустить hoyolab_watcher: {e}")
...
async def cmd_subscribe(message: types.Message):
    ...
    if message.chat.id in subs:
        await message.answer("Вы уже подписаны на уведомления HoYoLAB.")
        return
    subs.append(message.chat.id)
    _save_subscribers(subs)
    await message.answer("✅ Вы подписались на уведомления HoYoLAB. Чтобы отписаться, введите /unsubscribe")
...
# === Команда последняя новость ===

@dp.message(Command("lastnews"))
async def cmd_last_news(message: types.Message):
    """Отправляет все новости, вышедшие ровно 2 дня назад (UTC)."""
    target_date = (datetime.utcnow() - timedelta(days=2)).date()
    await message.answer(f"Поиск новостей за {target_date.strftime('%d.%m.%Y')}…")
    try:
        from hoyolab_watcher import fetch_articles_for_date, _send_article  # type: ignore
        articles = await fetch_articles_for_date(target_date)
        if not articles:
            await message.answer("Новостей за выбранную дату не найдено.")
            return
        # Send each article content
        for art, details in articles:
            await _send_article(bot, [message.chat.id], art, details)
    except Exception as e:
        logging.error(f"[bot] Ошибка при отправке последних новостей: {e}")
