...
def hub_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🖼 Генерация карточек", callback_data="hub:card")],
...
async def cb_info_main(callback: types.CallbackQuery, state: FSMContext):
    info_text = (
        ...
        "Сейчас доступна только Honkai: Star Rail. Мы постепенно добавляем:<br/>• ресурсы для прокачки<br/>• анализ патчноутов<br/>• новые функции.\n"
...
def fetch_profile_data(uid: str):
    ...
    """
    api_urls = [
        f"https://enka.network/api/hsr/v2/info/{uid}?lang=ru",
        f"https://enka.network/api/hsr/v2/info/{uid}?lang=en",
    ]
    for url in api_urls:
        try:
            resp = requests.get(url, timeout=10)
            if resp.status_code == 200:
                data = resp.json()
                # Простая адаптация структуры
                profile = {
                    "player": {
                        "nickname": data.get("player", {}).get("nickname", "Игрок"),
                        "level": data.get("player", {}).get("level", 0),
                    },
                    "characters": []
                }
                # characters may be in data["characters"] or data["avatars"] etc.
                chars_raw = data.get("characters") or data.get("avatars") or []
                for ch in chars_raw:
                    char_name = ch.get("name", ch.get("characterName", "?"))
                    portrait_id = ch.get("id") or ch.get("avatarId")
                    lc = ch.get("lightCone", {}).get("name") if ch.get("lightCone") else "-"
                    # реликты – соберём уникальные setName
                    relic_sets = []
                    if ch.get("relics"):
                        for r in ch["relics"]:
                            set_name = r.get("setName") or r.get("name")
                            if set_name and set_name not in relic_sets:
                                relic_sets.append(set_name)
                    stats_txt = ""
                    if ch.get("properties"):
                        props = {p.get("name") or p.get("propType"): p.get("value") for p in ch["properties"]}
                        atk = int(props.get("Attack", 0))
                        cr = props.get("CriticalChance", 0)
                        cd = props.get("CriticalDamage", 0)
                        stats_txt = f"АТК {atk} • КРИТ {cr}%/{cd}%"
...
async def msg_card_uid(message: types.Message, state: FSMContext):
    ...
    if not uid.isdigit():
        await message.reply("UID должен содержать только цифры.")
        return
    await message.answer("Генерирую карточку, пожалуйста, подождите…")
    await send_uid_card(uid, message.chat.id)
    await state.clear()

# --- Helper для отправки карточки по UID (используется /uid и кнопкой) ---
async def send_uid_card(uid: str, chat_id: int):
    profile_data = fetch_profile_data(uid)
    if not profile_data or not profile_data.get("characters"):
        await bot.send_message(chat_id, "Не удалось получить данные. Возможно профиль скрыт или API недоступно.")
        return
    ch = profile_data["characters"][0]
    build_data = {
        "title": f"{ch['name']}  •  Уровень {ch['level']}",
        "level": f"UID {uid} — {profile_data['player'].get('nickname')}",
        "light_cone": f"Свет. конус: {ch['light_cone']}",
        "relics": "Реликвии: " + (", ".join(ch['relic_sets']) if ch['relic_sets'] else "-"),
        "stats": ch.get("stats_text", ""),
    }
    art_path = None
    if ch.get("portrait_id"):
        cand = os.path.join("StarRailRes-master", "image", "character_preview", f"{ch['portrait_id']}.png")
        if os.path.exists(cand):
            art_path = cand
    if not art_path:
        art_path = get_art_path(ch['name']) or ""
    img = generate_card(build_data, art_path)
    bio = io.BytesIO()
    bio.name = "card.png"
    img.save(bio, "PNG")
    bio.seek(0)
