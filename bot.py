...
def format_best_build(build):
    return "\n".join([
        build.get("best_relic_pretty", ""),
        build.get("alt_relic_pretty", ""),
        build.get("best_5_lc_pretty", ""),
        build.get("best_4_lc_pretty", ""),
        build.get("best_planar_pretty", ""),
        build.get("alt_planar_pretty", ""),
        build.get("main_stats_pretty", ""),
        build.get("analytics_pretty", ""),
        build.get("recommended_substats_pretty", ""),
        build.get("best_teammates_pretty", ""),
        build.get("team_pretty", ""),
        build.get("role_pretty", "")
    ])
...
async def cb_choose_character(callback: types.CallbackQuery, state: FSMContext):
    ...
    if builds:
        for build in builds:
            await callback.message.answer(format_best_build(build), reply_markup=build_keyboard())
        return
    # Fallback: старая логика, если билдов нет
    await callback.message.answer("Билд не найден в базе best_builds.json.")
