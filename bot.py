...
@dp.errors()
async def error_handler(event: types.Update, error: Exception):
    try:
...
def format_items(items):
    """Форматирует список реликвий/конусов в более читаемый вид"""
    if not items:
        return ""
    formatted = []
    for item in items:
        # Разбиваем строку на рейтинг и описание
        parts = item.split(" - ", 1)
        if len(parts) == 2:
            rating, desc = parts
            # Экранируем специальные символы
            desc = desc.replace("-", "\\-").replace("_", "\\_").replace("*", "\\*")
            formatted.append(f"{rating} - {md.quote(desc)}")
        else:
            # Экранируем специальные символы
            item = item.replace("-", "\\-").replace("_", "\\_").replace("*", "\\*")
            formatted.append(md.quote(item))
    return "\n".join(formatted)

def split_stats(stats):
    """Разделяет характеристики на рекомендованные и приоритетные"""
    recommended = []
    priority = []
    for stat in stats:
        # Если есть числа или проценты - рекомендованная
        if any(char.isdigit() for char in stat) or "%" in stat:
            recommended.append(stat)
        else:
...
def format_build_v2(character_name, build):
    ...
    # Реликвии
    best_relic = extract_best(build.get("relics_recommended", []))
    if best_relic:
        lines.append("\n*Рекомендуемые реликвии:*")
        lines.append(format_items([best_relic]))
    
    alt_relic = extract_alternative(build.get("relics_alternative", []))
    if alt_relic:
        lines.append("\n*Альтернативные реликвии:*")
        lines.append(format_items([alt_relic]))
    ...
    if best_5:
        lines.append("\n*Конус 5★:*")
        lines.append(format_items([best_5]))
    
    alt_5 = extract_alternative(cones.get("5", []))
    if alt_5 and alt_5 != best_5:
        lines.append("\n*Альтернативный конус 5★:*")
        lines.append(format_items([alt_5]))
    
    best_4 = extract_best(cones.get("4", []))
    if best_4:
        lines.append("\n*Конус 4★:*")
        lines.append(format_items([best_4]))
    
    alt_4 = extract_alternative(cones.get("4", []))
    if alt_4 and alt_4 != best_4:
        lines.append("\n*Альтернативный конус 4★:*")
        lines.append(format_items([alt_4]))
    
    # Характеристики
    main_stats = build.get("main_stats", [])
    sub_stats = build.get("sub_stats", [])
    all_stats = main_stats + sub_stats
    recommended_stats, priority_stats = split_stats(all_stats)
    if recommended_stats:
        lines.append("\n*Рекомендованные характеристики:*")
        lines.append("\n".join(md.quote(s) for s in recommended_stats))
    if priority_stats:
        lines.append("\n*Приоритетные характеристики:*")
        lines.append(", ".join(md.quote(s) for s in priority_stats))
