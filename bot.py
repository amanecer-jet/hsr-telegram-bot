...
async def fetch_build_from_game8(character_name: str) -> dict:
    ...
    if relics_header:
        ...
        if section:
            if section.name == "ul":
                build["relics"] = [li.get_text(strip=True) for li in section.find_all("li")]
    ...
    if cones_header:
        ...
        for cone in cones_raw:
            ...
            if cone_stripped.startswith("★★★★★") or "5★" in cone_stripped or "5*" in cone_stripped or "5-star" in cone_stripped or "signature" in cone_stripped.lower():
                build["cones"]["5"].append(cone)
            elif cone_stripped.startswith("★★★★☆") or "4★" in cone_stripped or "4*" in cone_stripped or "4-star" in cone_stripped:
                build["cones"]["4"].append(cone)
            else:
                build["cones"]["other"].append(cone)
    # --- Stats ---
    stats_header = find_section(["Stats", "Characteristics", "Характеристики"])
    if stats_header:
    ...
    # --- Planar ---
    planar_header = find_section(["Planar", "Планарные украшения"])
    if planar_header:
        section = planar_header.find_next(["ul", "table"])
        if section:
            if section.name == "ul":
                build["planar"] = [li.get_text(strip=True) for li in section.find_all("li")]
            elif section.name == "table":
                for row in section.find_all("tr"):
                    cells = row.find_all("td")
                    if cells:
                        build["planar"].append(cells[0].get_text(strip=True))
    # --- Tips ---
...
def format_build_v2(character_name, build):
    ...
    best_relic = extract_best(relics)
    alt_relic = extract_alternative(relics)
    if best_relic:
    ...
    recommended_stats, priority_stats = split_stats(stats)
    if recommended_stats:
        lines.append(f"\n*Рекомендованные характеристики:*\n" + "\n".join(md.quote(s) for s in recommended_stats))
    if priority_stats:
        lines.append(f"\n*Приоритетные характеристики:*\n" + ", ".join(md.quote(s) for s in priority_stats))
    
    # Планарные украшения
    planar = build.get("planar", [])
    if planar:
        lines.append(f"\n*Планарные украшения:*\n" + "\n".join(md.quote(s) for s in planar))
