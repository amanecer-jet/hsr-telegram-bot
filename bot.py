...
async def fetch_build_from_game8(character_name: str) -> dict:
    ...
    # Light Cones
    cones_header = find_section(["Light Cones", "Световые конусы"])
    if cones_header:
    ...
    if stats_header:
        ...
        if section:
            if section.name == "ul":
                build["stats"]["main"] = {
                    "body": extract_best([li.get_text(strip=True) for li in section.find_all("li")]),
                    "feet": extract_alternative([li.get_text(strip=True) for li in section.find_all("li")])
                }
            elif section.name == "table":
                rows = section.find_all("tr")
                if len(rows) >= 4:
                    build["stats"]["main"] = {
                        "body": rows[0].find_all("td")[0].get_text(strip=True),
                        "feet": rows[1].find_all("td")[0].get_text(strip=True),
                        "sphere": rows[2].find_all("td")[0].get_text(strip=True),
                        "rope": rows[3].find_all("td")[0].get_text(strip=True)
                    }
            
            # Извлекаем приоритетные характеристики
            priority_header = find_section(["Priority Stats", "Приоритетные характеристики"])
            if priority_header:
                ...
                if section:
                    if section.name == "ul":
                        for li in section.find_all("li"):
                            text = li.get_text(strip=True)
                            # Извлекаем рейтинг из текста (например, "★★★★★ - Break Effect")
                            if "★" in text:
                                rating = text.count("★")
                                stat = text.split(" - ")[-1]
    ...
    if planar_header:
        ...
        if section:
            ...
            build["ornaments"]["best"] = extract_best(ornaments)
            build["ornaments"]["alternative"] = extract_alternative(ornaments)
