...
async def fetch_characters_and_elements() -> dict:
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Создаем новую страницу с настройками
            ...
            await page.goto(CHARACTER_BASE_URL, wait_until="networkidle")
            
            # Создаем скриншот для отладки
            try:
                await page.screenshot(path="debug_page.png", full_page=True)
                logging.info("Создан скриншот страницы для отладки")
            except Exception as e:
                logging.error(f"Не удалось создать скриншот: {str(e)}")
            
            # Ждем загрузки контента
            logging.info("Ждем загрузки контента...")
            try:
                ...
                if "Loading..." in html or "No characters found" in html:
                    logging.error("Страница не загрузилась полностью")
            ...
            characters_container = soup.find("div", class_="characters-page-nikke")
