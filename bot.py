...
async def fetch_build_from_game8(character_name: str) -> dict:
    ...
    if cones_header:
        ...
        for cone in cones_raw:
            if "5★" in cone or "5*" in cone or "5-star" in cone or "5★" in cone:
                build["cones"]["5"].append(cone)
            elif "4★" in cone or "4*" in cone or "4-star" in cone or "4★" in cone:
...
def extract_best(items):
    """Возвращает первый элемент из списка, если он есть, иначе пустую строку."""
    return items[0] if items else ""

def extract_alt(items):
    """Возвращает второй элемент из списка, если он есть, иначе пустую строку."""
...
def format_build_v2(character_name, build):
    ...
    alt_relic = extract_best(build.get("relics_alternative", []))
    if best_relic:
