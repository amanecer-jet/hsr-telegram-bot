...
def format_build(build):
    name = escape_markdown(build.get("name", ""))
    build_type = escape_markdown(build.get("build_type", ""))
    weapon = escape_markdown(build.get("weapon", ""))
    artifacts = build.get("artifacts", {})
    art_str = "\n".join([f"*{escape_markdown(k)}*: {escape_markdown(v)}" for k, v in artifacts.items()])
    return (
        f"*{name}*\n"
        f"_{build_type}_\n"
        f"*Оружие*: {weapon}\n"
