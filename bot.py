...
def game_keyboard():
    kb = [
        ...
        [InlineKeyboardButton(text="🖼 Карточка по UID", callback_data="card:start")],
...
async def msg_card_uid(message: types.Message, state: FSMContext):
    ...
    if not uid.isdigit():
        await message.reply("UID должен состоять только из цифр, попробуйте ещё раз или /cancel")
        return
