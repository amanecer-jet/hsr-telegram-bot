...
async def cmd_last_news(message: types.Message):
    """Отправляет все новости, вышедшие за последние 2 дня."""
    start_dt = datetime.utcnow() - timedelta(days=2)
    await message.answer("Поиск последних новостей за последние 2 дня…")
    try:
        from hoyolab_watcher import fetch_recent_articles, _send_article  # type: ignore
        articles = await fetch_recent_articles(days_back=2)
        if not articles:
